...
const revision = createCatalogRevision("finalize-scenarios-test-revision");
...
describe("representative creator finalization scenarios", () => {
  ...
  it("2024 preserves ability allocation and proficiency selections while serializing only authoritative state", () => {
    ...
    const result = finalizeCharacterWithCatalog(value, entities, revision)!; const roundTrip = deserializeCharacter(serializeCharacter(result));
