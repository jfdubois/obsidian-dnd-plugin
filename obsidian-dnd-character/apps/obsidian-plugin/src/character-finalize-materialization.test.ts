...
import { createCatalogRevision, createChoiceDefinitionId, createChoiceInstanceId, createChoiceOptionId, createEntityId, createRuleGrantId, createSourceId } from "@obsidian-dnd/domain";
...
const revision = createCatalogRevision("finalize-materialization-test-revision");
...
describe("catalog-aware creator finalization", () => {
  it("does not require legacy global buckets for review or materialization", () => {
    ...
    expect(buildReviewSnapshot(value, entities, revision)).not.toBeNull();
    expect(finalizeCharacterWithCatalog(value, entities, revision)).not.toBeNull();
  });
  ...
  it("materializes active item, named-item, fixed and resolved dice grants without mutating the draft or catalog", () => {
    ...
    const result = finalizeCharacterWithCatalog(value, entities, revision);
  ...
  it("materializes only the selected equipment-or-gold package and persists only active choices", () => {
    ...
    const equipmentResult = finalizeCharacterWithCatalog(value, entities, revision);
    ...
    const goldResult = finalizeCharacterWithCatalog(value, entities, revision);
  ...
  it("rejects active blockers before construction while ignoring inactive historical values", () => {
    ...
    const result = finalizeCharacterWithCatalog(value, [species(staleSpeciesId, []), background(), classRule(), item()], revision);
