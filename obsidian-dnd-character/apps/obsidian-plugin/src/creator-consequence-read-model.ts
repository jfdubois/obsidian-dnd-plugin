...
export async function loadCreatorConsequenceReadModel(
  ...
  const itemDetails = await Promise.all(itemIndex.filter((summary) => itemIds.has(summary.id))
    .map(async (summary) => {
      try { return (await catalog.fetchEntity(revision, summary.id, summary.detailPath)).data; }
      catch { return undefined; }
    }));
  const completeEntities = [...entities, ...itemDetails.filter((detail): detail is EntityDetailResponse => detail !== undefined)];
