...
import { createClassFeatureRule, type ClassFeatureRule } from "@obsidian-dnd/catalog-contract";
import type { RenderNode } from "@obsidian-dnd/catalog-contract";

export type ClassFeatureNormalizerDiagnosticCode =
  | "EXCLUDED_SOURCE"
  | "INVALID_SOURCE"
  | "UNKNOWN_SOURCE"
...
export interface ClassFeatureNormalizerDiagnostic {
  readonly code: ClassFeatureNormalizerDiagnosticCode;
...
export interface ClassFeatureNormalizerInput {
  readonly records: readonly CopyModRawRecord[];
  readonly context: ClassSourceScopeContext;
}

export interface ClassFeatureNormalizerResult {
  readonly features: readonly ClassFeatureRule[];
  readonly diagnostics: readonly ClassFeatureNormalizerDiagnostic[];
}
...
function makeDiagnostic(
  code: ClassFeatureNormalizerDiagnosticCode,
  message: string,
  recordName: string,
  opts: NormalizerOptions,
  source?: string,
): ClassFeatureNormalizerDiagnostic {
  const isError = [
    "INVALID_SOURCE",
    "INVALID_CANONICAL_ID",
    "MISSING_CLASS_NAME",
...
type SingleResult =
  | { readonly ok: true; readonly feature: ClassFeatureRule; readonly diagnostics: readonly ClassFeatureNormalizerDiagnostic[] }
  | { readonly ok: false; readonly diagnostics: readonly ClassFeatureNormalizerDiagnostic[] };

function normalizeSingleClassFeature(
  record: CopyModRawRecord,
  opts: NormalizerOptions,
): SingleResult {
  const diagnostics: ClassFeatureNormalizerDiagnostic[] = [];
  ...
  if (className === undefined) {
    diagnostics.push(makeDiagnostic(
      "MISSING_CLASS_NAME",
      `Class feature "${record.name}" is missing a valid className reference.`,
      record.name,
      opts,
    ));
    return { ok: false, diagnostics: Object.freeze(diagnostics) };
  ...
  if (level === undefined) {
    diagnostics.push(makeDiagnostic(
      "MISSING_LEVEL",
      `Class feature "${record.name}" is missing a valid level.`,
      record.name,
      opts,
    ));
    return { ok: false, diagnostics: Object.freeze(diagnostics) };
  ...
  if (!parentIdResult.ok) {
    diagnostics.push(makeDiagnostic(
      "INVALID_PARENT_ID",
      `Class feature "${record.name}" has an invalid parent class reference "${className}": ${parentIdResult.diagnostic.message}`,
      record.name,
      opts,
    ));
    return { ok: false, diagnostics: Object.freeze(diagnostics) };
  ...
  const feature = createClassFeatureRule(
    idResult.id,
    record.name,
    idResult.sourceId,
    scopeResult.ruleset,
...
export function normalizeClassFeatures(input: ClassFeatureNormalizerInput): ClassFeatureNormalizerResult {
  const features: ClassFeatureRule[] = [];
  const diagnostics: ClassFeatureNormalizerDiagnostic[] = [];

  for (let i = 0; i < input.records.length; i++) {
    const record = input.records[i];
    if (record === undefined) continue;
    const result = normalizeSingleClassFeature(record, {
      context: input.context,
      sourcePath: undefined,
      entityKind: "class-feature",
      recordIndex: i,
    });
