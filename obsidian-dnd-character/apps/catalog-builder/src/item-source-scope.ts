...
/* ── Supported item source registry ────────────────────────────── */
...
export type SupportedItemSource = SupportedItemSourceEntry["source"];
export type SupportedItemRuleset = SupportedItemSourceEntry["ruleset"];

const SUPPORTED_SOURCE_MAP: ReadonlyMap<string, SupportedItemSourceEntry> = new Map(
  SUPPORTED_ITEM_SOURCES.map((entry) => [entry.source, entry]),
);

const SUPPORTED_SOURCE_NAMES: readonly string[] = Object.freeze(
  SUPPORTED_ITEM_SOURCES.map((entry) => entry.source),
);

/* ── Types ─────────────────────────────────────────────────────── */

export interface ItemSourceScopeContext {
  readonly knownPinnedSources: FrozenReadonlySet<string>;
}

export interface ItemSourceScopeInput {
...
export type ItemSourceScopeDiagnosticCode =
  | "INVALID_SOURCE"
  | "UNSUPPORTED_ITEM_SOURCE"
  | "UNKNOWN_SOURCE";

export interface ItemSourceScopeDiagnostic {
  readonly code: ItemSourceScopeDiagnosticCode;
...
export interface ItemSourceScopeFailure {
  readonly ok: false;
  readonly diagnostic: ItemSourceScopeDiagnostic;
}

export type ItemSourceScopeResult = ItemSourceScopeSuccess | ItemSourceScopeFailure;

/* ── Diagnostic builder ────────────────────────────────────────── */
function buildDiagnostic(
  input: ItemSourceScopeInput,
  code: ItemSourceScopeDiagnosticCode,
  message: string,
  source?: string,
): ItemSourceScopeDiagnostic {
...
function lookupSupportedEntry(source: string): SupportedItemSourceEntry | undefined {
  return SUPPORTED_SOURCE_MAP.get(source);
}

export function classifyItemSourceScope(
  input: ItemSourceScopeInput,
  context: ItemSourceScopeContext,
): ItemSourceScopeResult {
  ...
  if (source.length === 0 || source !== source.trim()) {
    return Object.freeze({
      ...
      diagnostic: buildDiagnostic(
        input,
        "INVALID_SOURCE",
        "Item record has a malformed source abbreviation (empty or whitespace-padded).",
        source,
      ),
    });
  }
    ...
    if (!rulesetResult.ok) {
      return Object.freeze({
        ...
        diagnostic: buildDiagnostic(
          input,
          "UNKNOWN_SOURCE",
          `Ruleset classifier could not classify supported item source "${source}".`,
          source,
        ),
      });
    }
  ...
  return Object.freeze({
    ...
    diagnostic: buildDiagnostic(
      input,
      "UNKNOWN_SOURCE",
      `Source "${source}" is not recognized as a valid item source.`,
      source,
    ),
  });
}
