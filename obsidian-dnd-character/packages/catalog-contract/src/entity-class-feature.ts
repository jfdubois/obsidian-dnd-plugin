...
/* ── ClassFeatureRule ────────────────────────────────────────────
   Complete class feature definition extending RuleEntity fields with
   class-feature-specific data: parent class reference, level at which
   the feature is granted, and narrative content. Mechanical effects
   are projected from semantic mappings when available.              */

export interface ClassFeatureRule {
  id: EntityId;
  kind: "class-feature";
  name: string;
  sourceId: SourceId;
...
/* ── ClassFeatureRule validator ────────────────────────────────── */

export function isClassFeatureRule(value: unknown): value is ClassFeatureRule {
  if (typeof value !== "object" || value === null) return false;
  const obj = value as Record<string, unknown>;

  if (!isEntityId(obj.id)) return false;
  if (obj.kind !== "class-feature") return false;
  ...
  if (obj.level < 1 || obj.level > 20) return false;
...
export function createClassFeatureRule(
  id: EntityId,
  name: string,
  sourceId: SourceId,
  ruleset: Ruleset,
