...
/* ── SubclassFeatureRule ─────────────────────────────────────────
   Complete subclass feature definition extending RuleEntity fields
   with subclass-feature-specific data: parent subclass reference,
   owning class name, and level at which the feature is granted.    */

export interface SubclassFeatureRule {
  id: EntityId;
...
/* ── SubclassFeatureRule validator ─────────────────────────────── */

export function isSubclassFeatureRule(value: unknown): value is SubclassFeatureRule {
  if (typeof value !== "object" || value === null) return false;
  const obj = value as Record<string, unknown>;

  if (!isEntityId(obj.id)) return false;
  if (obj.kind !== "subclass-feature") return false;
...
export function createSubclassFeatureRule(
  id: EntityId,
  name: string,
  sourceId: SourceId,
  ruleset: Ruleset,
